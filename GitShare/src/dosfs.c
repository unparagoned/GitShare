...
//fast read file, this gets the first sector then just reads the following sectors, it's much faster but may have problems with
//unligned and or other stuff
/*
uint32_t Fast_DFS_ReadFile(PFILEINFO fileinfo, uint8_t *scratch, uint8_t *buffer, uint32_t *successcount, uint32_t len)
{
...
//				result = DFS_ReadSector(fileinfo->volinfo->unit, buffer, sector, 300);
	//			return result;
