...
SD_Error SD_LCDReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	while(DMAComplete==0);
	...
	SDIO_ITConfig(SDIO_IT_DATAEND , ENABLE);
	SD_LCD_DMA_RxConfig(readbuff, (NumberOfBlocks * BlockSize));
	SDIO_DMACmd(ENABLE);
	//SD_LowLevel_DMA_RxConfig((uint32_t *)readbuff, (NumberOfBlocks * BlockSize));
...
SD_Error SD_FASTLCDReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	  ...
	  SDIO_DMACmd(ENABLE);
	  //SD_LowLevel_DMA_RxConfig((uint32_t *)readbuff, (NumberOfBlocks * BlockSize));
	 SD_FASTLCD_DMA_RxConfig(readbuff, (NumberOfBlocks * BlockSize));
