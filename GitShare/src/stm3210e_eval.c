...
void SD_LCD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
	//hmm code seems a bit buggy, bufferdst sometimes didn't work well.
	uint32_t tempHolder;
	//tempHolder=BufferDST;
//	BufferDST=((uint32_t)0x60020000);
//BufferDST=tempHolder;
//BufferDST=(uint32_t)BufferDST;
//BufferDST=(uint32_t)0x60000000;
	numberOfCycles=BufferSize/262144+(BufferSize%262144?1:0);
	//  numberOfCycles+=(BufferSize%262144?1:0);
	DMA_InitTypeDef DMA_InitStructure;
	  ...
	  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)BufferDST;
	  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	  DMA_InitStructure.DMA_BufferSize = BufferSize/(4*numberOfCycles);//63744;//BufferSize/4;//0xFFFF;// BufferSize/4;
	  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	  //DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
...
void SD_FASTLCD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
	//hmm code seems a bit buggy, bufferdst sometimes didn't work well.
	uint32_t tempHolder;
	//tempHolder=BufferDST;
...
//BufferDST=(uint32_t)0x60000000;
	  ...
	  /*!< DMA2 Channel4 Config */
		  ...
		  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)BufferDST;
		  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
		  DMA_InitStructure.DMA_BufferSize = BufferSize/4;//0xFFFF;// BufferSize/4;
		  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
		  //DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
		  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Disable;
